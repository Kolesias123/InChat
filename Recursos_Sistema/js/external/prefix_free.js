...
var self = window.StyleFix = {
	...
	link: function(link) 
	{
		...
		if(strpos(url, 'infosmart.mx') == false && strpos(url, 'localhost') == false)
			return;

		if(strpos(document.URL, 'Setup') !== false)
			return;
		   
		// Cross Origin.
		url = Path + '/Kernel/FakeProxy.php?toUrl=' +  Base64.encode(url);
		...
		xhr.onreadystatechange = function() {
			if(xhr.readyState === 4) {
				...
				if(css && link.parentNode) {
					...
					setTimeout(function() { parent.removeChild(link) }, 500);
				}
			}
		};
